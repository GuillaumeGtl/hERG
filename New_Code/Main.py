...
for i in range(len(pre_aa)):
    ...
    if type(clash[i])==float and math.isnan(clash[i]):
        ...
        f.close()
        subprocess.run(cmd)
        clash[i],mini[i] = get_clash(mutation)
        clash[i] = [int(j) for j in clash[i]]
        mini[i] = [int(j) for j in mini[i]]
        write_xlsx(config["mutation_file"],mutation[:-4],"clash",clash[i])
        write_xlsx(config["mutation_file"],mutation[:-4],"minimisation",mini[i])
    if type(c4[i])==float and math.isnan(c4[i]):
        c4[i] = crit4(mutation)
        write_xlsx(config["mutation_file"],mutation[:-4],"critère4",c4[i])
    if type(total[i])==float and math.isnan(total[i]):
        total[i] = c1[i]+c2[i]+c3[i]+c4[i]
        write_xlsx(config["mutation_file"],mutation[:-4],"total",total[i])
    if type(AA_imp[i])==float and math.isnan(AA_imp[i]):
        AA_imp[i] = [aa for aa in get_contactAA(mutation) if aa in AA_important]
        write_xlsx(config["mutation_file"],mutation[:-4],"AA_important en contact",AA_imp[i])
